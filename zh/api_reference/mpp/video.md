...
- [kd_mpi_venc_request_chn](#kd_mpi_venc_request_chn)：申请编码通道。
- [kd_mpi_venc_release_chn](#kd_mpi_venc_release_chn)：释放编码通道。
- [kd_mpi_venc_attach_vb_pool](#kd_mpi_venc_attach_vb_pool)：绑定VB池到编码通道。
- [kd_mpi_venc_detach_vb_pool](#kd_mpi_venc_detach_vb_pool)：解除编码通道的VB池绑定。
- [kd_mpi_venc_create_chn](#kd_mpi_venc_create_chn)：创建编码通道。
- [kd_mpi_venc_destroy_chn](#kd_mpi_venc_destroy_chn)：销毁编码通道。
...
- [kd_mpi_venc_set_rotation](#kd_mpi_venc_set_rotation)：设置编码图像旋转角度。
- [kd_mpi_venc_get_rotation](#kd_mpi_venc_get_rotation)：获取编码图像旋转角度。
...
| 参数名称 | 描述                                                                       | 输入/输出 |
| -------- | -------------------------------------------------------------------------- | --------- |
| chn_num  | 输出申请到的编码通道号，取值范围：[0,[VENC_MAX_CHN_NUMS](#venc_max_chn_nums))。 | 输出      |
...
- [kd_mpi_vdec_request_chn](#kd_mpi_vdec_request_chn)：申请解码通道。
- [kd_mpi_vdec_release_chn](#kd_mpi_vdec_release_chn)：释放解码通道。
- [kd_mpi_vdec_attach_vb_pool](#kd_mpi_vdec_attach_vb_pool)：绑定VB池到解码通道。
- [kd_mpi_vdec_detach_vb_pool](#kd_mpi_vdec_detach_vb_pool)：解除解码通道的VB池绑定。
...
| 参数名称 | 描述                                                                       | 输入/输出 |
| -------- | -------------------------------------------------------------------------- | --------- |
| chn_num  | 输出申请到的解码通道号，取值范围：[0,VDEC_MAX_CHN_NUMS] | 输出      |
...
| 参数名称 | 描述                                                                     | 输入/输出 |
| -------- | ------------------------------------------------------------------------ | --------- |
| chn_num  | 解码通道号。 取值范围：[0,VDEC_MAX_CHN_NUMS] | 输入      |

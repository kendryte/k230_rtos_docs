...
| 参数名称 | 描述 | 输入/输出 |
|---|---|---|
| chn_num  | 输出申请到的空闲通道号，取值范围：[0, K_NONAI_2D_MAX_CHN_NUMS]。 | 输出      |
...
- 如果chn_num为空指针，返回K_ERR_NONAI_2D_NULL_PTR。
- 若无空闲通道，返回K_ERR_NONAI_2D_NOBUF或对应无资源错误码。
- 申请到的通道需要通过kd_mpi_nonai_2d_create_chn初始化后才能使用。
...
| 参数名称 | 描述 | 输入/输出 |
|---|---|---|
| chn_num  | 需要释放的通道号，取值范围：[0, K_NONAI_2D_MAX_CHN_NUMS]。 | 输入      |
...
- 如果通道号超出合法范围，返回K_ERR_NONAI_2D_INVALID_CHNID。
- 如果通道未被申请或已被释放，返回K_ERR_NONAI_2D_UNEXIST。
- 释放通道前需确保通道已停止（kd_mpi_nonai_2d_stop_chn）且销毁（kd_mpi_nonai_2d_destroy_chn），否则返回操作不允许错误。
...
- 如果通道号非法，返回K_ERR_NONAI_2D_INVALID_CHNID。
- 如果通道未创建，返回K_ERR_NONAI_2D_UNEXIST。
- 如果VB池ID无效，返回K_ERR_NONAI_2D_ILLEGAL_PARAM。
- 一个通道同一时间只能绑定一个VB池，重复绑定会覆盖原有绑定关系。

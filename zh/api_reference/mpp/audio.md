...
- **音频编码和解码模块（AENC 和 ADEC）**：音频编解码模块（AENC/ADEC）为音频数据处理提供标准化编解码能力，原生支持 G711、Opus 两大主流音频格式，可满足不同场景下的音频编码、解码需求，同时具备灵活的扩展能力，适配多样化的业务应用场景。
G711 为业界经典通用音频编码标准，采用脉冲编码调制（PCM）压缩方式，编解码算法简单、处理时延低，且兼容性极强，广泛适配各类基础音频通信、采集回放场景，运行稳定性高，无需复杂配置即可快速集成使用；
Opus 为新一代开源高效音频编码格式，兼顾低时延与高压缩比，支持窄带至全频带的音频编码，在低码率下仍能保持优异的音频音质，同时适配实时语音通话、音频流传输等对时延和音质均有要求的中高端应用场景，灵活性与实用性突出。
模块设计遵循可扩展架构，提供标准化的开放扩展接口，支持用户根据业务需求注册自定义外部编解码器，快速拓展更多音频格式的编解码能力，打破原生格式限制，充分满足各类差异化、定制化的音频处理需求，适配更丰富的行业应用场景。
...
音频编码主要实现创建编码通道、发送音频帧编码及获取编码码流等功能。音频编码部分，提供g711a/u、opus编码，暂只支持16bit采样精度。
...
AENC 模块最大可注册 20 个编码器，且自身已注册 G711.a、G711.u、opus 编码器。
...
音频编解码部分，提供g711a/u、opus解码，暂支持16bit采样精度。
...
用户通过传入解码器属性结构体，向 ADEC 模块注册一个解码器，并返回注册句柄，用户可以最后通过注册句柄来注销该解码器。 ADEC 模块最大可注册 20 个解码器，且自身已注册 G711a、G711u、opus解码器。 同一种解码协议不允许重复注册解码器，例如假如已注册 G711 解码器，不允许另外再注册一个 G711 解码器。
...
协议类型指定了该通道的解码协议，目前支持 G711、opus。音频解码的部分属性需要与输出设备属性相匹配，例如采样率、帧长（每帧采样点数目）等。 buffer 大小以帧为单位，取值范围是[2, [K_MAX_AUDIO_FRAME_NUM](#k_max_audio_frame_num)，建议配置为 10 以上，过小的 buffer 配置可能导致丢帧等异常。 在通道未创建前（或销毁后）才能使用此接口，如果通道已经被创建，则返回通道已经创建。

...
音频模块由以下几个子模块组成：音频输入（AI）、音频输出（AO）、音频编码（AENC）、音频解码（ADEC），以及对内置模拟音频编解码器的支持。音频输入模块支持I2S和PDM两种接口，结合PDMA进行内存拷贝，能够处理数字麦克风（PDM/I2S）和模拟音频编解码器（I2S）的输入信号。内置模拟音频编解码器支持对音频数据进行声音质量增强（VQE）处理，包括回声消除（AEC）、降噪（ANR）和自动增益控制（AGC）。音频输出模块支持I2S接口，通过PDMA进行内存拷贝，可以连接外部数字扬声器或模拟音频编解码器进行音频输出。音频编码和解码模块目前支持G711格式的音频编解码，并允许注册外部编解码器。

音频模块被拆分为多个子模块，通过低耦合的方式实现数据流的传递，以满足MAIX3平台上的多媒体业务需求。下图展示了在语音对讲业务场景下音频模块的使用方式：音频输入模块采集音频数据，通过音频编码模块进行编码后通过网络发送到远端，远端接收到的音频编码数据经过音频解码模块解码后，通过音频输出模块播放远端的声音。
...
音频输入模块支持I2S和PDM协议接口。I2S支持同时采集最多2路双声道音频，PDM支持同时采集最多8路单声道音频。详细的I2S、PDM特性如下：
...
- 音频编解码器将模拟麦克风接收到的信号转换为I2S格式的PCM数据，并输入到音频模块的I2S接口中；I2S输出的PCM数据经过音频编解码器处理后，转换为模拟信号输出。此模式不使用数字IO，固定使用I2S的sdi0和sdo0接口。

- I2S接口可以直接连接片外的数字麦克风和功率放大器（PA）。共有两组接口可供选择：sdi0、sdo0及sdi1、sdo1。

- 片外的PDM麦克风最多可以通过音频模块的4个输入数据接口输入8路PDM数据。

可供选择的链路包括：

- 3组pdm_in + 1组i2s_in + 2组i2s_out，其中1组i2s_in可以使用内置音频编解码器或片外设备
- 4组pdm_in + 2组i2s_out
- 2组i2s_in + 2组pdm_in + 2组i2s_out

可以使用内置编解码器或外接设备（音频子板）来测试音频相关功能。使用内置编解码器可以测试一组I2S音频输入和输出及音频编解码器相关功能，使用音频子板可以测试2组I2S音频输入输出和4组PDM音频输入功能。

![img](https://kendryte-download.canaan-creative.com/developer/pictures/b76dc39c5256a09c53be0e7572c2f587.png)

#### 声音质量增强

音频模块支持对音频数据进行声音质量增强处理，实现音频3A功能（自动增益控制、回声消除和噪声抑制）。目前支持的采样精度为16位。

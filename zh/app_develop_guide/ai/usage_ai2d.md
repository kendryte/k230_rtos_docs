...
`AI2D` 是基于硬件加速的图像预处理模块，用于在开发板端完成推理前的数据处理，以降低 CPU 开销并提升整体性能。 `AI2D` 提供了 **5 种基于硬件加速的图像预处理能力**，分别为 **Crop / Shift / Resize / Pad / Affine**。
在目标检测等视觉任务中，可通过组合 `Resize` 与 `Pad` 实现常见的 **letterbox** 预处理流程。

`AI2D` 主要用于 **开发板端的部署阶段**，以加速预处理过程，提高推理速度。其 **C++ 运行时接口** 详见文档：[AI2D 运行时 API 手册](../../api_reference/nncase/ai2d_runtime.md)
...
在该目录下执行 `build_app.sh` 即可完成编译，生成的可执行文件位于 `k230_bin` 目录。
将生成物拷贝至开发板后即可运行示例程序。

在使用 `AI2D` 时，请务必注意以下事项。

> **注意：**
>
> 1. **`Affine` 与 `Resize` 功能互斥**，二者不可同时启用；
> 1. **`Shift` 功能仅支持 `Raw16` 输入格式**；
> 1. **`Pad` 的填充值按通道配置**，其列表元素个数需与输入图像的通道数一致；
> 1. 在当前版本中，**即使仅使用 AI2D 的单一功能，其余功能参数仍需配置**，但只需将对应 `flag` 置为 `false`，其余字段可不填写；
> 1. 当同时配置多个功能时，**AI2D 的执行顺序固定为**：
>
>    ```bash
>    Crop → Shift → Resize / Affine → Pad
>    ```
>
>    请确保各阶段的参数配置在尺寸、格式等方面保持一致。

## 预处理方法

### Resize 方法

`Resize` 方法是一种在图片预处理中广泛使用的操作，它主要用于改变图像的尺寸大小。无论是放大还是缩小图像，都可以通过这个方法来实现。源码位于 `src/rtsmart/examples/ai/usage_ai2d/test_resize` 目录下。实现`Resize` 的流程如下：
...
`Crop` 方法是一种用于从原始图像中提取（裁剪）出感兴趣区域（ROI，Region of Interest）的操作。它可以根据指定的坐标和尺寸，从图像中选取一部分作为新的图像。源码位于 `src/rtsmart/examples/ai/usage_ai2d/test_crop` 。实现 `Crop` 的流程如下：
...
`Pad`（填充）方法是一种在图片预处理阶段，对图像边缘进行填充操作的技术。它通过在图像的四周（上下左右）添加像素值，来改变图像的尺寸大小。这些添加的像素值可以自定义。源码位于 `src/rtsmart/examples/ai/usage_ai2d/test_pad` 。实现 `Pad` 的流程如下：
...
`Affine`（仿射变换）方法是一种在图像预处理中用于对图像进行几何变换的技术。它可以实现图像的旋转、平移、缩放等多种几何变换操作，并且能够保持图像的 “平直性”（即直线在变换后仍然是直线）和 “平行性”（即平行的线在变换后仍然平行）。源码位于 `src/rtsmart/examples/ai/usage_ai2d/test_affine` 。实现 `Affine` 的流程如下：
...
`Shift` 方法是数据预处理中比特位右移的方法，每右移一位原数据变为原来的1/2。需要注意的是 `Shift` 的输入数据格式必须为 `RAW16` 。源码位于 `src/rtsmart/examples/ai/usage_ai2d/test_shift` 。实现 `Affine` 的流程如下：

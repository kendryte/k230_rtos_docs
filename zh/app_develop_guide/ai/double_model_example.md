...
在文档[单模型应用开发指南](./single_model_example.md)中，我们介绍了如何开发一个单模型推理的 AI 应用并在 k230 上运行。 那么对于多模型推理的场景，我们也可以参考单模型的开发指南进行开发。这里以人脸识别为例介绍双模型推理应用的开发。和单模型应用的区别在于他需要实现多模型的加载和配置。

## 开发指南

### 转换kmodel

首先是要有一个`kmodel`，对于人脸识别我们可以使用已经准备好的 `src/rtsmart/examples/ai/face_recognition` 中 `utils` 目录下的 `face_detection_320.kmodel` 和`face_recognition.kmodel`。
...
AI双模型应用开发的基本开发结构同样可以采用**单摄像头双通道处理**的方式。其核心思路是将摄像头采集到的图像分为两路处理：

- **一路图像**直接绑定到屏幕进行显示，以保证画面能够**实时、低延迟地呈现**；
- **另一路图像**则用于**AI 模型推理**，即将图像转换为 tensor，送入模型进行处理，得到检测和识别的结果。
...
如下图所示，为双模型串行推理时单摄双通道处理逻辑的流程图：

![double_model_pipeline](https://www.kendryte.com/api/post/attachment?id=840)

如下图所示，为双模型并行推理时单摄双通道处理逻辑的流程图：

![double_model_pipeline](https://www.kendryte.com/api/post/attachment?id=841)

> Note: 这里需要注意，KPU推理是独占过程，如果使用多线程实现并行推理需要加同步锁处理，避免多个线程同时访问KPU导致冲突。

#### 代码结构介绍

以人脸识别双模型推理为例，下面是已有的代码结构：
...
│    ├── face_recognition.cc  # 人脸识别任务场景实现，包括适应该场景模型的前处理、推理、后处理、结果绘制等，还包括人脸数据库初始化，新增人脸，人脸计数，人脸匹配等接口
│    ├── face_recognition.h   # 人脸识别任务场景头文件
...
|face_recognition.h|提供具体任务场景（这里是人脸识别）的预处理、推理、后处理、结果绘制等接口，还包括人脸数据库初始化，新增人脸，人脸计数，人脸匹配等接口|
|face_recognition.cc|提供了face_recognition.h定义的任务场景接口实现|
...
- `face_detection.h`、 `face_detection.cc`、 `face_recognition.h`、 `face_recognition.cc`和 `main.cc` 是用户在开发新的AI应用时**需要自行编写**的文件，编写参考 `src/rtsmart/examples/ai/face_recognition` 下的对应文件即可。其中任务场景的头文件和实现文件主要**实现该任务模型的输入前处理、推理（一般直接调用`ai_base.h`中的`run`方法），模型后处理部分的代码**；`main.cc` 文件需要修改模型推理部分的逻辑，包括**具体任务场景类的实例初始化，前处理、模型推理、后处理和结果绘制接口的调用**。
...
`face_detection.h`、 `face_detection.cc`、 `face_recognition.h`、 `face_recognition.cc` 是**用户在二次开发时需要自行实现的核心文件**。
...
在这两个文件中，你需要实现一个**任务场景类（Task Class）**，该类必须：
...
这里假设应用场景类的头文件和实现为`myapp.h`和`myapp.cc`，其中`myapp.h`的结构可以仿照`face_detection.h`和`face_recognition.h`编写，这里以串行推理为例给出伪代码：
...
上述定义的接口需要在`myapp.cc`中做具体实现，此处不再赘述。您可以参考`src/rtsmart/examples/ai/face_recognition/src`中的代码仿写。
...
![double_model_inference_rtos](https://www.kendryte.com/api/post/attachment?id=838)
...
比如对于人脸识别任务源码目录中的`src/CMakeLists.txt`，需要修改添加需要编译的子目录，这里将源码拆分成了两个CMakeLists.txt，也可以合成一个，对这部分不熟悉的用户可以忽略：

```cmake
add_subdirectory(src) 
```

对于人脸检测任务子目录内的 `face_recognition/src/CMakeLists.txt`，需要修改待编译的文件和生成的可执行文件elf名称：
...
编译脚本文件`face_recognition/build_app.h`定义了编译使用的环境变量，同时需要修改elf文件拷贝路径：
...
执行结束后，在`output`目录下会生成编译的镜像。我们希望用户可以将应用代码放到`src/rtsmart/examples/ai`目录下，您可以参照该目录下的`face_recognition`实现。
...
在`RTOS SDK`根目录下执行`make menuconfig`，选择`RT-Smart UserSpace Examples Configuration`->`Enable build ai examples`->`Enable Build Face Recognition Programs`，保存并退出。如下图：

![rtos_facerec_menuconfig](https://www.kendryte.com/api/post/attachment?id=839)
...
这样部署汇总文件会在编译过程中直接编译到固件中的`/sdcard/app/examples/ai/face_recognition`目录下。也可以**直接进入到对应的目录下**执行：
...
然后使用串口工具连接开发板，在命令行执行`run.sh`脚本，注意参数要和代码中的位置和类型相符。

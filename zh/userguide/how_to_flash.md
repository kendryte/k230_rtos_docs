...
本文档默认你已经完成编译，并在 `output/<defconfig>/images/` 下拿到镜像文件。

常见镜像文件类型：

- `*.kdimg`：推荐用于 K230BurningTool（USB）
- `*.img`：推荐用于 SD 卡写盘，也可用于部分命令行烧录工具
...
- USB 烧录：直接连接设备快速更新固件，推荐 `K230BurningTool`，也可使用 `K230-Flash`
- SD 卡烧录：无 USB 接口或批量写卡场景，推荐 `Rufus` 或 `balenaEtcher`
